...
if st.query_params["user_id"]:
    ...
    correct_data_per_date = correct_data.groupby(correct_data['tanggal'].dt.date).size().reset_index(name='jumlah')

    # Line chart jumlah gerakan sesuai per tanggal
    st.header(f'Jumlah Gerakan Sesuai per Tanggal untuk: {user_name}')
    fig = px.line(correct_data_per_date, x='tanggal', y='jumlah', title='Jumlah Gerakan Sesuai per Tanggal', labels={'tanggal': 'Tanggal', 'jumlah': 'Jumlah Gerakan Sesuai'})
    st.plotly_chart(fig)
